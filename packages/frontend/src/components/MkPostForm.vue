...
<template>
	...
	>
		...
		<datalist id="hashtags">
			...
			<option v-for="hashtag in flavors" :key="hashtag" :value="hashtag"/>
		</datalist>
		<datalist id="coffees">
			<option v-for="hashtag in coffees" :key="hashtag.source" :value="hashtag.label">{{ hashtag.label }}</option>
		</datalist>
	</div>
	</template>
	...
	let checkinRating = $ref<number|null>(3);
	let coffeeBrandLogo = $ref<string|null>(null);
	let coffeeBrandSource = $ref<string|null>(null);
	let coffeeBrandClass = $ref<string|null>(null);
	...
	async function post(ev?: MouseEvent) {
		...
		os.api(postData.editId ? "notes/edit" : "notes/create", postData, token).then(() => {
			...
			nextTick(() => {
				...
				if ((lowerCase.includes('love') || lowerCase.includes('❤')) && lowerCase.includes('beantown')) {
	...
	function updateBranding() {
		console.log('ub')
		const db = coffees.find(c => c.label === checkinCoffee);
		console.log('ub1', db)
		if (db) {
			coffeeBrandLogo = db.icon;
			coffeeBrandSource = db.source;
			coffeeBrandClass = db.classification;
		}
	};
	</script>
